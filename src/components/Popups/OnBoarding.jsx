...
const OnBoarding = () => {
  ...
  const halfStyle =
    "w-[48%] py-[10px] px-[10px] bg-[#252525] text-[#fff] rounded-md";
  const socialStyle =
    "w-full px-[10px] py-[10px] rounded-md bg-[#252525] text-[#fff]";
  const submitData = async () => {
    ...
    const dataRes = await sendData("/api/v1/auth/signup", form)
      ...
      .catch((error) => {
        console.log(error);
        setErrorMsg(
          `${error.response.data.message} - Please try Github Username`
        );
      });
  };
  ...
  return (
    <div className="w-[50vw] h-full flex justify-center items-end rounded-md">
      <div className="w-[full] h-full bg-[#1a1a1a] flex flex-col justify-start items-center gap-2 px-2 pt-6 py-4">
        ...
        <h1 className="font-[800] text-[30px]">
          Create Your{" "}
          <span className="px-[20px] py-[5px] text-black bg-primary rounded-sm">
            {" "}
            Profile
          </span>
        </h1>
        <form className="flex flex-col w-full items-center gap-[10px] mt-[20px] mb-[50px]">
          ...
          <div className="w-full flex justify-between">
            ...
            <input
              ...
              name="email"
              disabled={true}
              title="Email Once set cannot be changed"
              className={`${halfStyle}`}
            />
          </div>
          <input
          ...
          <textarea
            ...
            name="bio"
            className={`${socialStyle} h-[100px] resize-none`}
            onChange={changeHandler}
          ></textarea>
